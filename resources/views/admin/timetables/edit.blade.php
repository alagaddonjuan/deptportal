        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.timetable.update', $schedule) }}">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="day_of_week" id="day_of_week" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm" required>
                                    @foreach ($daysOfWeek as $id => $day)
                                        <option value="{{ $id }}" @selected(old('day_of_week', $schedule->day_of_week) == $id)>{{ $day }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <!-- Location -->
                             <div>
                                ...
                                <x-text-input id="location" class="block mt-1 w-full" type="text" name="location" :value="old('location', $schedule->location)" required />
                            </div>
                        ...
                        <div class="flex items-center justify-end mt-6">
                            ...
                            <x-primary-button class="ms-4">Update Entry</x-primary-button>
                        </div>
                    </form>
                </div>
            </div>
